...
public class CompleteBowlingGameSteps implements En {
    ...
    public CompleteBowlingGameSteps(InMemoryBowlingGameContainer inMemoryBowlingGameContainer) {
        ...
        When("^turn number is ([0-9]|10) and the bowling player scored (.) and (.)$", (Integer turnNumber, String firstRollScore, String secondRollScore) -> {
            inMemoryBowlingGameContainer.getBowlingGame().roll(turnNumber, 0, firstRollScore.charAt(0));
            inMemoryBowlingGameContainer.getBowlingGame().roll(turnNumber, 1, secondRollScore.charAt(0));
        ...
        And("^the roll (.) & (.) should be valid$", (String firstRollScore, String secondRollScore) -> {
            Assert.assertTrue(inMemoryBowlingGameContainer.getBowlingGame().isRollValid(firstRollScore.charAt(0)));
            Assert.assertTrue(inMemoryBowlingGameContainer.getBowlingGame().isRollValid(secondRollScore.charAt(0)));
        });
